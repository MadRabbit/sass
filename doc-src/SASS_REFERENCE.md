...
those groups dynamically, without needing variable interpolation. They have no
direct parallel in CSS, although they're syntactically similar to media query
expressions:
...
a map. The [`@each` directive](#each-destructuring) can be used to add styles
...
The `@each` directive usually has the form `@each $var in <list>`.
...
#### Destructuring {#each-destructuring}
...
Since [maps](#maps) are treated as lists of pairs, destructing works with them
as well. For example:
