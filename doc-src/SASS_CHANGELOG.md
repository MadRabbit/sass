...
be used with `@each`, using the new destructuring feature (see below):
...
* `@each` now supports "destructuring", which makes it easier to iterate over
  lists of lists. If you write `@each $var1, $var2 in a b, c d, d e`, the
  elements of the sub-lists will be assigned individually to the variables.
  `$var1` and `$var2` will be `a` and `b`, then `c` and `d`, and then `e` and
